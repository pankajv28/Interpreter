...
class SymbolTableVisitor(NodeVisitor):
    ...
    def visit_VarDecl(self,node):
        ...
        self.symtable.define(VarSymbol(var_name,type_name))

    def visit_Assign(self,node):
        left = node.left.value
        symbol_value = self.symtable.lookup(left)
        if symbol_value is not None :
            self.visit (node.right)
        else : 
            raise NameError (f'{left} doesnt exist')
        
    def visit_Var(self,node):
        print('var')
        symbol_name = self.symtable.lookup(node.value)
    ...
    def visit_Type(self,node):
        data_type = node.token.type
        if self.symtable.lookup(data_type) is None :
...
def main():
        ...
        symtab_builder = SymbolTableVisitor()
        symtab_builder.visit(tree)
        print('')
        print('Symbol Table contents:')
        print(symtab_builder.symtable)
